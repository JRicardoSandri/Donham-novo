...
import CAMPAIGN_HTML from '../data/campaignHtml.js';
import {
  gameTerm,
  itemDescription,
  itemName,
...
export default function SoloCampaignScreen({ language = 'pt-BR' }) {
  ...
  const charactersJson = useMemo(
    () => JSON.stringify(characters).replace(/</g, '\\u003c'),
    [characters]
  );
  const injectedCharacters = useMemo(() => {
    const lang = JSON.stringify(String(language || 'pt-BR'));
    return `window.__RPGCT_LANG__ = ${lang}; window.__RPGCT_CHARACTERS__ = ${charactersJson}; true;`;
  }, [charactersJson, language]);


  const pushCharacters = useCallback(() => {
    if (!webRef.current) return;
    const lang = JSON.stringify(String(language || 'pt-BR'));
    webRef.current.injectJavaScript(
      `window.__RPGCT_SET_CHARACTERS && window.__RPGCT_SET_CHARACTERS(${charactersJson}, ${lang}); true;`
    );
  }, [charactersJson, language]);

  /* Empurra os personagens sempre que mudarem — resolve o "carregando" na 1ª entrada
     (quando a lista ainda não estava pronta no load inicial da WebView). */
  useEffect(() => { pushCharacters(); }, [pushCharacters]);
  ...
  return (
    <View style={styles.screen}>
      <WebView
        ref={webRef}
        originWhitelist={['*']}
        source={{ html: CAMPAIGN_HTML }}
        injectedJavaScriptBeforeContentLoaded={injectedCharacters}
        onLoadEnd={pushCharacters}
        onMessage={onMessage}
        javaScriptEnabled
        domStorageEnabled
        allowFileAccess
...
const styles = StyleSheet.create({
  ...
  webView: { flex: 1, backgroundColor: '#08090C' },
});
